  ...
  return {
    ...
    */
    init: function() {
      ...
      var hideLineWrap = '.js-settline-toggle';
      $('.admin-center').on('click', '.section-settings .debugSave', function() { 
        admin.ajaxRequest({
          mguniqueurl: "action/setDebugVars",   
          data: {
            debugDisablePlugins: $('.section-settings #debugDisablePlugins').prop('checked'),
      ...
      $('.admin-center').on('change', '.section-settings #q2-openGraph', function() { 
        // для Opengraph
      ...
      $('.admin-center').on('click', '.section-settings .tabs-title-settings', function() { 
        ...
        if($('.table-with-arrow').is('visible')) {
          admin.showSideArrow();
        }
      });
      ...
      $('body').on('change', '#tuseCaptcha, #tcaptchaOrder, #tuseReCaptcha, [name=searchType], #tusePhoneMask, #tmainPageIsCatalog, #tprintAgreement', function() {
        if ($('#tuseCaptcha').prop('checked') || $('#tcaptchaOrder').prop('checked')) {
          $('[name=useReCaptcha]').parents(hideLineWrap).show();
        }
        else{
          $('#tuseReCaptcha').prop('checked',false);
          $('[name=useReCaptcha]').parents(hideLineWrap).hide();
          $('[name=reCaptchaKey]').parents(hideLineWrap).hide();
          $('[name=reCaptchaSecret]').parents(hideLineWrap).hide();
        }
        if ($('#tuseReCaptcha').prop('checked')) {
          $('[name=reCaptchaKey]').parents(hideLineWrap).show();
          $('[name=reCaptchaSecret]').parents(hideLineWrap).show();
          $('[name=invisibleReCaptcha]').parents(hideLineWrap).show();
        }
        else{
          $('[name=reCaptchaKey]').parents(hideLineWrap).hide();
          $('[name=reCaptchaSecret]').parents(hideLineWrap).hide();
          $('[name=invisibleReCaptcha]').parents(hideLineWrap).hide();
        }
        if ($('[name=searchType]').val() == 'sphinx') {
          $('[name=searchSphinxHost]').parents(hideLineWrap).show();
          $('[name=searchSphinxPort]').parents(hideLineWrap).show();
          $('[name=sphinxLimit]').parents(hideLineWrap).show();
        }
        else{
          $('[name=searchSphinxHost]').parents(hideLineWrap).hide();
          $('[name=searchSphinxPort]').parents(hideLineWrap).hide();
          $('[name=sphinxLimit]').parents(hideLineWrap).hide();
        }
        if ($('#tusePhoneMask').prop('checked')) {
          $('[name=phoneMask]').parents(hideLineWrap).show();
        }
        else{
          $('[name=phoneMask]').parents(hideLineWrap).hide();
        }
        ...
        if ($('#tmainPageIsCatalog').prop('checked')) {
          $('[name=randomProdBlock]').parents(hideLineWrap).show();
          $('[name=countNewProduct]').parents(hideLineWrap).show();
          $('[name=countRecomProduct]').parents(hideLineWrap).show();
          $('[name=countSaleProduct]').parents(hideLineWrap).show();
        }
        else{
          $('[name=randomProdBlock]').parents(hideLineWrap).hide();
          $('[name=countNewProduct]').parents(hideLineWrap).hide();
          $('[name=countRecomProduct]').parents(hideLineWrap).hide();
          $('[name=countSaleProduct]').parents(hideLineWrap).hide();
        }
      });
      ...
      $('.admin-center').on('click', '.section-settings #tab-wholesale', function() {
        settings.openTab($(this).attr('id'));
        if (window.rebootWholesale === true) {
      ...
      // переход во вкладку "Дополнительные поля"
      $('.admin-center').on('click', '.section-settings #tab-opfields', function() {   
        includeJS(admin.SITE + '/mg-core/script/admin/opfields.js'); 
        settings.openTab($(this).attr('id'));
      });

      // переход во вкладку "Форма заказа"
      $('.admin-center').on('click', '.section-settings #tab-orderform', function() {
        settings.openTab($(this).attr('id'));        
        if (window.rebootOrderForm === true) {
          window.rebootOrderForm = false;
          admin.refreshPanel();
        }
        includeJS(admin.SITE + '/mg-core/script/admin/orderform.js');
      });
      ...
      $('.admin-center').on('click', '.section-settings #tab-1C', function() {   
        includeJS(mgBaseDir + '/mg-core/script/admin/log1c.js'); 
        settings.openTab($(this).attr('id'));
      });
      ...
      $('.admin-center').on('change', '.section-settings  select[name="cacheMode"]', function() {
        ...
        $('input[name="cacheHost"]').parents(hideLineWrap).hide();
        $('input[name="cachePort"]').parents(hideLineWrap).hide();
        $('input[name="cachePrefix"]').parents(hideLineWrap).hide();
        if($(this).val()=="MEMCACHE") {
          $('.memcache-conection').show();	
          $('input[name="cacheHost"]').parents(hideLineWrap).show();
          $('input[name="cachePort"]').parents(hideLineWrap).show();
          $('input[name="cachePrefix"]').parents(hideLineWrap).show();
        }
      });
      ...
      $('.admin-center').on('click', '.section-settings  .browseOpenGraphLogo', function() {
        admin.openUploader('settings.getOgLogo'); 
        $('.logo-og-img').show();      
      });
    ...
    //Обработка  нажатия кнопки Приступить к обновлению
      ...
      $('.admin-center').on('click', '.section-settings .remove-added-logo-og', function() {
        $(this).hide();
      ...
      $('.admin-center').on('change', '.section-settings  input[name="smtp"]', function() {
        ...
        if($(this).val()=="true") {
          $('.section-settings input[name=smtpSsl]').parents(hideLineWrap).show();
          $('.section-settings input[name=smtpHost]').parents(hideLineWrap).show();
          $('.section-settings input[name=smtpLogin]').parents(hideLineWrap).show();
          $('.section-settings input[name=smtpPass]').parents(hideLineWrap).show();
          $('.section-settings input[name=smtpPort]').parents(hideLineWrap).show();
        } else {
          $('.section-settings input[name=smtpSsl]').parents(hideLineWrap).hide();
          $('.section-settings input[name=smtpHost]').parents(hideLineWrap).hide();
          $('.section-settings input[name=smtpLogin]').parents(hideLineWrap).hide();
          $('.section-settings input[name=smtpPass]').parents(hideLineWrap).hide();
          $('.section-settings input[name=smtpPort]').parents(hideLineWrap).hide();
        }	
      });
      ...
      $('.admin-center').on('click', '.section-settings .email-conection', function() {           
       ...
       $('.admin-center').on('change', '.section-settings #tab-shop-settings input[name=sessionToDB]', function() {
         if ($(this).val() == 'true') {
           $('.section-settings #tab-shop-settings input[name=sessionLifeTime]').parents(hideLineWrap).show();
         } else {
           $('.section-settings #tab-shop-settings input[name=sessionLifeTime]').parents(hideLineWrap).hide();
         }
       });
      ...
      $('.admin-center').on('change', '[name=sessionToDB]', function() {
        if($(this).prop('checked')) settings.checkInnoSuport();
      });
    },
     ...
     var paramArray = JSON.parse($('tr[id=payment_'+data.id+'] td#paramHideArray').text());     
      ...
      if('' != $('tr[id=payment_'+data.id+'] td#urlArray').text()) {
        ...
        $.each(urlArray, function(name, val) {
          ...
          if(k==3) {urlParam += '<p class="alert-block success">'}
          if(k==4) {urlParam += '<p class="alert-block alert">'}
          if(k==5) {urlParam += '<p class="alert-block refund">'}
          if(k==6) {urlParam += '<p class="alert-block refund">'}
